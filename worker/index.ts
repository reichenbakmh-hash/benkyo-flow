...
//           activeSessionsNow, users: [...] } · 401 code admin incorrect ·
//     503 fonctionnalité non configurée.
//
...
async function handleAuth(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  if (action === "admin-stats" && request.method === "POST") {
    ...
    const totals = await env.DB.prepare(
      ...
      .first<{ total_users: number; new_7d: number; new_30d: number; active_sessions: number }>();
