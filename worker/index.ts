...
async function handleAuth(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  if (action === "admin-stats" && request.method === "POST") {
    ...
    // Activité "réelle" = au moins une ligne créée dans une des tables de
    // contenu (le schéma n'a pas de colonne updated_at, donc on ne peut pas
    // détecter une simple modification — seulement une création).
    const activityUnion = `
      SELECT user_id, created_at FROM subjects
      UNION ALL SELECT user_id, created_at FROM homework
      UNION ALL SELECT user_id, created_at FROM goals
      UNION ALL SELECT user_id, created_at FROM notions
      UNION ALL SELECT user_id, created_at FROM study_sessions
    ...
    const totals = await env.DB.prepare(
      `SELECT
         ...
         (SELECT COUNT(DISTINCT user_id) FROM (${activityUnion}) WHERE created_at >= ?) AS active_users_7d,
         (SELECT COUNT(DISTINCT user_id) FROM (${activityUnion}) WHERE created_at >= ?) AS active_users_30d,
      ...
      .bind(sevenDaysAgo, thirtyDaysAgo, now, sevenDaysAgo, thirtyDaysAgo)
      .first<{
        total_users: number;
        new_7d: number;
        new_30d: number;
    ...
    const usersResult = await env.DB.prepare(
      `SELECT
         ...
         (SELECT MAX(c) FROM (
           SELECT created_at AS c FROM subjects WHERE user_id = u.id
           UNION ALL SELECT created_at FROM homework WHERE user_id = u.id
           UNION ALL SELECT created_at FROM goals WHERE user_id = u.id
           UNION ALL SELECT created_at FROM notions WHERE user_id = u.id
           UNION ALL SELECT created_at FROM study_sessions WHERE user_id = u.id
           UNION ALL SELECT created_at FROM events WHERE user_id = u.id
         )) AS last_activity_at
       FROM users u
    ...
    return json({
      ...
      activeUsersLast7Days: totals?.active_users_7d ?? 0,
      activeUsersLast30Days: totals?.active_users_30d ?? 0,
