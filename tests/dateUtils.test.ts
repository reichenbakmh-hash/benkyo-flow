...
import { daysUntil, isWithinDays, isoDateDaysAgo, computeStudyStreak, nextRecurrenceDate } from "../src/lib/dateUtils.ts";
...
describe("nextRecurrenceDate", () => {
  test("quotidien, intervalle 1", () => {
    assert.equal(nextRecurrenceDate("2026-01-10", { frequency: "daily", interval: 1 }), "2026-01-11");
  });

  test("quotidien, intervalle 3", () => {
    assert.equal(nextRecurrenceDate("2026-01-10", { frequency: "daily", interval: 3 }), "2026-01-13");
  });

  test("hebdomadaire, intervalle 1", () => {
    assert.equal(nextRecurrenceDate("2026-01-10", { frequency: "weekly", interval: 1 }), "2026-01-17");
  });

  test("hebdomadaire, intervalle 2", () => {
    assert.equal(nextRecurrenceDate("2026-01-10", { frequency: "weekly", interval: 2 }), "2026-01-24");
  });

  test("mensuel, intervalle 1", () => {
    assert.equal(nextRecurrenceDate("2026-01-10", { frequency: "monthly", interval: 1 }), "2026-02-10");
  });

  test("mensuel, franchit une fin d'année", () => {
    assert.equal(nextRecurrenceDate("2026-12-15", { frequency: "monthly", interval: 1 }), "2027-01-15");
  });

  test("intervalle non entier arrondi à l'entier inférieur, minimum 1", () => {
    assert.equal(nextRecurrenceDate("2026-01-10", { frequency: "daily", interval: 0.5 }), "2026-01-11");
  });

  test("intervalle négatif traité comme 1", () => {
    assert.equal(nextRecurrenceDate("2026-01-10", { frequency: "daily", interval: -5 }), "2026-01-11");
  });

  test("date invalide renvoie null", () => {
    assert.equal(nextRecurrenceDate("pas-une-date", { frequency: "daily", interval: 1 }), null);
  });
});
