...
// Modale "Me contacter" : ouverte en cliquant sur le logo Benkyō Flow dans
// la barre latérale. Liens vers les réseaux/comptes personnels du créateur.
const CONTACT_LINKS = [
  {
    label: "WhatsApp",
    value: "+261 38 30 897 21",
    href: "https://wa.me/261383089721",
    icon: Phone,
  ...
  },
  {
    label: "X (Twitter)",
    value: "@AzhellZettour",
    href: "https://x.com/AzhellZettour",
    icon: AtSign,
  ...
  },
  {
    ...
    href: "https://discord.gg/vD7C8Ng4",
    icon: MessageSquare,
...
function ContactModal({ onClose }: { onClose: () => void }) {
  return (
    <Modal title="Me contacter" onClose={onClose}>
      <p className="bf-contact__intro">
        <Heart size={14} /> Benkyō Flow est un projet personnel — n'hésite pas à me suivre ou à me faire
        signe si tu as des retours, des idées, ou juste envie de dire bonjour.
      </p>
      <div className="bf-contact__list">
        {CONTACT_LINKS.map((c) => (
          <a key={c.label} href={c.href} target="_blank" rel="noopener noreferrer" className="bf-contact__link">
            <span className={`bf-contact__icon tone-${c.tone}`}>
              <c.icon size={18} />
            </span>
            <span className="bf-contact__text">
              <span className="bf-contact__label">{c.label}</span>
              <span className="bf-contact__value">{c.value}</span>
            </span>
            <ExternalLink size={15} className="bf-contact__external" />
          </a>
        ))}
      </div>
    ...
    <div className="bf-app">
      <aside className={`bf-sidebar ${settings.sidebarCollapsed ? "collapsed" : ""} ${mobileSidebarOpen ? "expanded" : ""}`}>
        <div className="bf-sidebar__header">
          <button
            type="button"
            className="bf-sidebar__brand"
            onClick={() => setShowContactModal(true)}
            aria-label="Me contacter et suivre le projet"
            title="Me contacter et suivre le projet"
          >
