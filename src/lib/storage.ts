...
export interface HomeworkRecurrence {
...
export interface Homework {
  ...
  recurrence: HomeworkRecurrence | null;
  createdAt: number;
}
...
export type CalendarEventType = "exam" | "other";
...
import type { PomodoroPreset } from "./pomodoro";
...
export interface AppSettings {
  ...
  sidebarBackgroundOpacity: number; // 0-100, visibilité de l'image de fond du menu
  appBackgroundImage: string | null; // image personnalisée (data URL), en fond de toute la zone de contenu
  appBackgroundOpacity: number; // 0-100, visibilité de l'image de fond de l'application
  ...
  customPomodoroPresets: PomodoroPreset[];
...
export const defaultSettings: AppSettings = {
  ...
  sidebarBackgroundOpacity: 18,
  appBackgroundImage: null,
  appBackgroundOpacity: 7,
...
export function loadAppData(): AppData {
  ...
  return {
    ...
    subjects,
    homework: normalizeHomeworkList(safeGet<Homework[]>(KEYS.homework, [])),
...
function eventsKeyFor(userId: string | null): string {
  return userId ? `${STORAGE_PREFIX}events:${userId}` : KEYS.events;
}
...
const HOMEWORK_RECURRENCE_FREQUENCIES = ["daily", "weekly", "monthly"];

function isValidRecurrence(r: unknown): r is HomeworkRecurrence {
  if (!r || typeof r !== "object") return false;
  const rec = r as Record<string, unknown>;
  return HOMEWORK_RECURRENCE_FREQUENCIES.includes(rec.frequency as string) && typeof rec.interval === "number" && rec.interval > 0;
}

function normalizeHomeworkList(list: Homework[]): Homework[] {
  return list.map((h) => ({
    ...h,
    subtasks: Array.isArray(h.subtasks) ? h.subtasks : [],
    recurrence: isValidRecurrence(h.recurrence) ? h.recurrence : null,
  }));
}

export function loadHomeworkFor(userId: string | null): Homework[] {
  return normalizeHomeworkList(safeGet<Homework[]>(homeworkKeyFor(userId), []));
}
...
export function loadEventsFor(userId: string | null): CalendarEvent[] {
  return safeGet<CalendarEvent[]>(eventsKeyFor(userId), []);
}
export function saveEventsFor(userId: string | null, events: CalendarEvent[]) {
  safeSet(eventsKeyFor(userId), events);
}
...
export interface PendingSyncOp {
  key: string;
  kind: "subject" | "homework" | "goal" | "notion" | "studySession" | "event";
  action: "upsert" | "delete";
  id: string;
  label: string;
  payload?: unknown;
}
function pendingSyncKeyFor(userId: string | null): string {
  return `${STORAGE_PREFIX}pending-sync:${userId ?? "guest"}`;
}
export function loadPendingSyncFor(userId: string | null): PendingSyncOp[] {
  return safeGet<PendingSyncOp[]>(pendingSyncKeyFor(userId), []);
}
export function savePendingSyncFor(userId: string | null, ops: PendingSyncOp[]) {
  safeSet(pendingSyncKeyFor(userId), ops);
}

// Mémorise l'identifiant du dernier compte connecté sur cet appareil, dans
// une clé fixe (non scindée par utilisateur, contrairement au thème et aux
// réglages ci-dessus). Sert uniquement au script anti-flash de index.html :
// avant même que la session ne soit vérifiée côté React, il a besoin de
// savoir quelle clé de thème lire (générique en mode invité, ou
// "theme:<id>" pour un compte) — voir le commentaire dans index.html.
// Ne contient qu'un identifiant opaque, jamais de donnée personnelle.
const LAST_USER_KEY = `${STORAGE_PREFIX}last-user-id`;
export function rememberLastUserId(userId: string | null) {
  try {
    if (userId) {
    ...
    // Stockage indisponible : l'anti-flash retombera sur le mode invité au
    // prochain chargement, sans casser le fonctionnement de l'app.
  }
}
...
export function loadEvents(): CalendarEvent[] {
  return safeGet<CalendarEvent[]>(KEYS.events, []);
}
...
// Mélange deux couleurs (weightA = part de hexA, entre 0 et 1).
function mix(hexA: string, hexB: string, weightA: number): string {
  const [ar, ag, ab] = hexToRgb(hexA);
  const [br, bgc, bb] = hexToRgb(hexB);
  return rgbToHex(
    ar * weightA + br * (1 - weightA),
    ag * weightA + bgc * (1 - weightA),
...
//
// La couleur "surface" d'une ambiance est pensée comme un accent ponctuel
// (une carte qui ressort), pas comme le remplissage de la barre du haut, de
// tous les panneaux et de toutes les cartes à la fois — sinon l'interface
// entière devient une seule couleur saturée et illisible. On n'en garde donc
// qu'un léger reflet, posé sur un fond neutre dérivé du fond de page, plutôt
// que la couleur brute.
export function resolvePalette(p: FullPalette): ResolvedPalette {
  const dark = isDarkColor(p.bg);
  const card = p.surface
    ? mix(p.surface, p.bg, dark ? 0.32 : 0.14)
    : shade(p.bg, dark ? 0.07 : -0.03);
  const border = p.border ?? shade(card, dark ? 0.18 : -0.14);
...
async function apiRequest<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      if (data && typeof data.detail === "string") message += ` — détail : ${data.detail}`;
    } catch {
...
// Envoie un email contenant un lien de réinitialisation si le compte existe.
// La réponse est volontairement la même que l'email existe ou non (message
// générique renvoyé par le serveur), pour ne pas permettre à quelqu'un de
// deviner quels emails sont inscrits.
export async function apiForgotPassword(email: string): Promise<{ ok: boolean; message: string }> {
  return apiRequest<{ ok: boolean; message: string }>("/api/auth/forgot-password", {
    method: "POST",
    body: JSON.stringify({ email }),
  });
}

// Consomme le token reçu par email pour définir un nouveau mot de passe.
// Invalide aussi toutes les sessions existantes du compte côté serveur.
...
export interface AdminStats {
  ...
  totalStudyMinutesAll: number;
  totalHomeworkAll: number;
  totalNotionsAll: number;
  totalGoalsAll: number;
  totalEventsAll: number;
  totalSubjectsAll: number;
  signupsLast30Days: AdminSignupDay[];
  users: AdminUserStats[];
}

export async function apiAdminStats(adminAccessCode: string): Promise<AdminStats> {
  return apiRequest<AdminStats>("/api/auth/admin-stats", {
    method: "POST",
...
function parseHomeworkSubtasks(raw: string | null): HomeworkSubtask[] {
  if (!raw) return [];
  try {
...
function parseHomeworkRecurrence(raw: string | null): HomeworkRecurrence | null {
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw);
    return isValidRecurrence(parsed) ? parsed : null;
  } catch {
    return null;
  }
}
...
export async function apiListHomework(): Promise<Homework[]> {
  ...
  return data.results.map((r) => ({
    ...
    status: r.status,
    notes: r.notes ?? "",
    subtasks: parseHomeworkSubtasks(r.subtasks),
    recurrence: parseHomeworkRecurrence(r.recurrence),
    createdAt: r.created_at,
  }));
}
...
export async function apiListEvents(): Promise<CalendarEvent[]> {
  const data = await apiRequest<{ results: RawEvent[] }>("/api/events");
  return data.results.map((r) => ({
    id: r.id,
    subjectId: r.subject_id,
    title: r.title,
    date: r.event_date,
    type: r.type === "exam" ? "exam" : "other",
    notes: r.notes ?? "",
    createdAt: r.created_at,
  }));
}
export async function apiUpsertEvent(e: CalendarEvent): Promise<void> {
  await apiRequest("/api/events", { method: "POST", body: JSON.stringify(e) });
}
