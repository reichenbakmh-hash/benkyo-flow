...
export function clampPomodoroSettings(s: PomodoroSettings): PomodoroSettings {
  return {
    workMinutes: Math.min(180, Math.max(1, Math.round(s.workMinutes) || 25)),
    breakMinutes: Math.min(60, Math.max(1, Math.round(s.breakMinutes) || 5)),
    longBreakMinutes: Math.min(90, Math.max(1, Math.round(s.longBreakMinutes) || 15)),
    sessionsBeforeLongBreak: Math.min(10, Math.max(2, Math.round(s.sessionsBeforeLongBreak) || 4)),
  };
}
