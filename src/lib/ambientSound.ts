...
export class AmbientSoundEngine {
  ...
  // À appeler de façon synchrone depuis un vrai gestionnaire de clic/tap
  // (ex. le bouton "Démarrer") : c'est ce qui débloque l'audio sur
  // mobile (Safari iOS notamment), qui refuse silencieusement de créer ou
  // reprendre un AudioContext en dehors d'un geste utilisateur direct.
  // Un appel ultérieur depuis un useEffect (donc hors du geste) trouve
  // alors le contexte déjà actif.
  unlock() {
    this.ensureContext();
  }
