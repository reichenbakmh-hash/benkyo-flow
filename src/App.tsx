...
export default function App() {
  ...
  // --- Couleurs personnalisées : remplacent le thème par défaut si définies -
  useEffect(() => {
    ...
    <>
      ...
      <div className="bf-panel">
        ...
        <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid var(--bf-border)" }}>
          <p style={{ fontSize: 13.5, fontWeight: 600, marginBottom: 10 }}>Couleurs</p>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
            {colorPresets.map((p) => (
              <button
                ...
                title={p.label}
                onClick={() =>
                  onSettingsChange({ ...settings, customPrimaryColor: p.primary, customAccentColor: p.accent })
                }
                style={{
                  width: 40,
                  height: 40,
                  borderRadius: "50%",
                  border:
                    settings.customPrimaryColor === p.primary && settings.customAccentColor === p.accent
          ...
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "center" }}>
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13.5 }}>
              ...
              <input
                ...
                onChange={(e) => onSettingsChange({ ...settings, customPrimaryColor: e.target.value })}
            ...
            {(settings.customPrimaryColor || settings.customAccentColor) && (
              <button
                className="bf-btn ghost small"
                onClick={() => onSettingsChange({ ...settings, customPrimaryColor: null, customAccentColor: null })}
              >
                Réinitialiser
              </button>
            )}
      ...
      <div className="bf-panel">
        ...
        <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid var(--bf-border)" }}>
          ...
          <p style={{ fontSize: 13, color: "var(--bf-text-muted)", marginBottom: 12 }}>
            Mets une image en fond du menu (max 1,5 Mo).
          </p>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (file.size > 1.5 * 1024 * 1024) {
                    window.alert("Image trop lourde (max 1,5 Mo). Choisis une image plus légère.");
                    return;
                  }
                  const reader = new FileReader();
                  reader.onload = () => {
                    onSettingsChange({ ...settings, sidebarBackgroundImage: String(reader.result) });
                  };
