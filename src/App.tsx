...
import { daysUntil, isWithinDays, isoDateDaysAgo, computeStudyStreak } from "./lib/dateUtils";
import {
  checkDueReminders,
...
const CONTACT_LINKS = [
  ...
  { label: "Discord", href: "https://discord.gg/pAwgNbx5", icon: SiDiscord, tone: "discord" as const },
];
...
export default function App() {
  ...
  const [pendingDelete, setPendingDelete] = useState<{
    kind: "subject" | "homework" | "goal" | "notion";
    id: string;
    title: string;
    message: string;
  } | null>(null);
  ...
  function exportAllData() {
    ...
    logEvent("Données exportées en JSON");
  }
  ...
  function requestImportData(raw: unknown) {
    ...
    const total = impSubjects.length + impHomework.length + impGoals.length + impNotions.length + impSessions.length;
    if (total === 0) {
  ...
  function confirmPendingImport() {
    ...
    const { subjects: impSubjects, homework: impHomework, goals: impGoals, notions: impNotions, studySessions: impSessions } =
      pendingImport;
    ...
    if (authMode === "account") {
      impSubjects.forEach((s) => apiUpsertSubject(s).catch(handleCloudError));
      impHomework.forEach((h) => apiUpsertHomework(h).catch(handleCloudError));
      impGoals.forEach((g) => apiUpsertGoal(g).catch(handleCloudError));
      impNotions.forEach((n) => apiUpsertNotion(n).catch(handleCloudError));
      impSessions.forEach((s) => apiUpsertStudySession(s).catch(handleCloudError));
    }

    logEvent(`Import JSON : ${pendingImport.summary}`);
    showToast(`Import réussi : ${pendingImport.summary}.`, "success");
  ...
  function completeHomeworkFromToday(id: string) {
    setHomework((prev) =>
      prev.map((h) => {
        ...
        const updated: Homework = { ...h, status: "done" };
        logEvent(`Devoir terminé : ${h.title}`);
        if (authMode === "account") apiUpsertHomework(updated).catch(handleCloudError);
        return updated;
      })
    );
    ...
    <div className="bf-app">
      ...
      {pendingImport && (
        <ConfirmDialog
          ...
          message={`Le fichier contient : ${pendingImport.summary}. Les éléments dont l'id correspond à un élément existant seront mis à jour ; les autres seront ajoutés. Rien n'est supprimé par un import.`}
  ...
  function handleImportFileChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      try {
        ...
        onToast("Fichier invalide : ce n'est pas un JSON lisible.", "danger");
      }
    };
    ...
    <>
      ...
      <div className="bf-panel">
        ...
        <div className="bf-data-io">
          <p style={{ fontSize: 13.5, color: "var(--bf-text-muted)", marginBottom: 10 }}>
            Exporte une sauvegarde JSON de toutes tes données, ou importe-en une pour restaurer ou
            fusionner des données (rien n'est jamais supprimé par un import : un élément déjà existant
            est mis à jour, un nouveau est ajouté).
          </p>
