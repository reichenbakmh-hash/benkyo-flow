...
export default function App() {
  ...
  // --- Persistance : on sauvegarde à chaque changement ----------------------
  ...
  // --- Ecran de connexion simple (identification locale) --------------------
  if (!user.loggedIn) {
    return (
      <div className="bf-auth">
        <div className="bf-auth__card">
          ...
          <p className="bf-auth__subtitle">
            Ton espace d'organisation scolaire. Entre ton prénom pour commencer.
          </p>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            <div className="bf-field">
              <label htmlFor="bf-name">Prénom</label>
              <input
                id="bf-name"
                type="text"
            ...
            <button className="bf-btn primary" type="submit" style={{ width: "100%", justifyContent: "center" }}>
        ...
        <div className="bf-topbar">
          ...
          <div className="bf-topbar__actions">
            ...
            <div className="bf-user">
              <div className="bf-user__avatar">{user.name.slice(0, 1).toUpperCase()}</div>
              <span className="bf-user__name">{user.name}</span>
            </div>
            <button
              className="bf-logout"
              onClick={() => setUser((u) => ({ ...u, loggedIn: false }))}
            >
              Déconnexion
            </button>
          </div>
        ...
        {section === "goals" && (
          <GoalsSection
            ...
            onProgressChange={(id, progress) => {
              setGoals((prev) =>
                prev.map((g) => (g.id === id ? { ...g, progress, done: progress >= 100 } : g))
              );
            }}
          />
        )}
  ...
  return (
    <>
      ...
      <div className="bf-panel">
        ...
        <p style={{ fontSize: 13.5, color: "var(--bf-text-muted)", marginBottom: 10 }}>
          Tes données sont stockées localement sur cet appareil. Cette action supprime toutes tes matières,
          devoirs, objectifs et historique.
        </p>
        <button
